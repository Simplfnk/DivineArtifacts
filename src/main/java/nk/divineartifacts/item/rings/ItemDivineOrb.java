...
public class ItemDivineOrb extends DivineOrbBase {
	...
	public Multimap<Attribute, AttributeModifier> curioModifiers (
		...
		if (CuriosApi.getItemStackSlots(stack , slotContext.entity()).containsKey(slotContext.identifier()) && TSPELL_POWER.get() && configOrbOfMagic.get() && configOrbOfMagic.get()) {
		...
		if (CuriosApi.getItemStackSlots(stack , slotContext.entity()).containsKey(slotContext.identifier()) && TMANA_REGEN.get() && configOrbOfMagic.get()) {
			modifiers.put(AttributeRegistry.MANA_REGEN.get() ,
					new AttributeModifier(MANA_REGEN_UUID , "Mana Regen" , (double) MANA_REGEN.get() / 100 ,
							AttributeModifier.Operation.MULTIPLY_TOTAL));
		}
		if (CuriosApi.getItemStackSlots(stack , slotContext.entity()).containsKey(slotContext.identifier()) && TMAX_MANA.get() && configOrbOfMagic.get() && configOrbOfMagic.get()) {
			modifiers.put(AttributeRegistry.MAX_MANA.get() ,
					new AttributeModifier(MAX_MANA_UUID , "Max Mana" , (double) MAX_MANA.get() / 100 ,
							AttributeModifier.Operation.MULTIPLY_TOTAL));
		}
		if (CuriosApi.getItemStackSlots(stack , slotContext.entity()).containsKey(slotContext.identifier()) && TCOOLDOWN_REDUCTION.get() && configOrbOfMagic.get()) {
		...
		if (CuriosApi.getItemStackSlots(stack , slotContext.entity()).containsKey(slotContext.identifier()) && TCAST_TIME_REDUCTION.get() && configOrbOfMagic.get()) {
