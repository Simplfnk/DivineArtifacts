	...
	public Multimap<Attribute, AttributeModifier> curioModifiers(
		...
		if (CuriosApi.getItemStackSlots(stack , slotContext.entity()).containsKey(slotContext.identifier()) && TogBldTabBloodMagicResist.get() && this.isEnabled.get()) {
			modifiers.put(AttributeRegistry.BLOOD_MAGIC_RESIST.get() ,
					new AttributeModifier(BLOOD_MAGIC_RESIS_UUID , "" , ValBldTabBloodMagicResist.get() / 100.0 ,
		...
		if (CuriosApi.getItemStackSlots(stack , slotContext.entity()).containsKey(slotContext.identifier()) && TogBldBloodSpellPower.get() && this.isEnabled.get()) {
			modifiers.put(AttributeRegistry.BLOOD_SPELL_POWER.get() ,
					new AttributeModifier(BLOOD_SPELL_UUID , "" , ValBldTabBloodSpellPower.get() / 100.0 ,
							AttributeModifier.Operation.MULTIPLY_TOTAL));
		}
		if (CuriosApi.getItemStackSlots(stack , slotContext.entity()).containsKey(slotContext.identifier()) && TogBldTabCastTime.get() && this.isEnabled.get()) {
			modifiers.put(AttributeRegistry.CAST_TIME_REDUCTION.get()  ,
					new AttributeModifier(CAST_TIME_UUID , "" , ValBldTabCastTime.get() / 100.0 ,
							AttributeModifier.Operation.MULTIPLY_TOTAL));
		}
		if (CuriosApi.getItemStackSlots(stack , slotContext.entity()).containsKey(slotContext.identifier()) && TogBldTabCoolDown.get() && this.isEnabled.get()) {
			modifiers.put(AttributeRegistry.COOLDOWN_REDUCTION.get() ,
					new AttributeModifier(COOL_DAWON_UUID , "" , -ValBldTabCoolDown.get() / 100.0 ,
							AttributeModifier.Operation.MULTIPLY_TOTAL));
		}
