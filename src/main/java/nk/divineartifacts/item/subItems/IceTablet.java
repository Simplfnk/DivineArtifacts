...
public class IceTablet extends IceTabletBase {
	...
	public Multimap<Attribute, AttributeModifier> curioModifiers(
		...
		if (CuriosApi.getItemStackSlots(stack , slotContext.entity()).containsKey(slotContext.identifier()) && toggleIceTabIceMagicResist() && this.isEnabled.get()) {
			modifiers.put(AttributeRegistry.ICE_MAGIC_RESIST.get() ,
					new AttributeModifier(ICE_MAGIC_RESIST_UUID , "" , getIceTabIceMagicResist() / 100.0 ,
							AttributeModifier.Operation.MULTIPLY_TOTAL));
		}
		if (CuriosApi.getItemStackSlots(stack , slotContext.entity()).containsKey(slotContext.identifier()) && toggleIceTabIceSpellPower() && this.isEnabled.get()) {
			modifiers.put(AttributeRegistry.ICE_SPELL_POWER.get() ,
					new AttributeModifier(ICE_SPELL_POWER_UUID , "" , getIceTabIceSpellPower() / 100.0 ,
							AttributeModifier.Operation.MULTIPLY_TOTAL));
		}
		if (CuriosApi.getItemStackSlots(stack , slotContext.entity()).containsKey(slotContext.identifier()) && toggleIceTabMaxMana() && this.isEnabled.get()) {
			modifiers.put(AttributeRegistry.MAX_MANA.get() ,
					new AttributeModifier(MAX_MANA_UUID , "" , getIceTabMaxMana() ,
							AttributeModifier.Operation.ADDITION));
		}
		if (CuriosApi.getItemStackSlots(stack , slotContext.entity()).containsKey(slotContext.identifier()) && toggleIceTabNatureMagicResist() && this.isEnabled.get()) {
			modifiers.put(AttributeRegistry.NATURE_MAGIC_RESIST.get() ,
					new AttributeModifier(NATURE_MAGIC_RESIST_UUID , "" , -getIceTabNatureMagicResist() / 100.0 ,
							AttributeModifier.Operation.MULTIPLY_TOTAL));
		}
