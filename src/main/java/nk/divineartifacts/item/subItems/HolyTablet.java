...
public class HolyTablet extends HolyTabletBase {
	...
	public Multimap<Attribute, AttributeModifier> curioModifiers(
		...
		if (CuriosApi.getItemStackSlots(stack , slotContext.entity()).containsKey(slotContext.identifier()) && toggleHlyTabHollyMagicResist() && this.isEnabled.get()) {
			modifiers.put(AttributeRegistry.HOLY_MAGIC_RESIST.get() ,
					new AttributeModifier(HOLY_MAGIC_RISIST_UUID , "" , getHlyTabHollyMagicResist() / 100.0 ,
							AttributeModifier.Operation.MULTIPLY_TOTAL));
		}
		if (CuriosApi.getItemStackSlots(stack , slotContext.entity()).containsKey(slotContext.identifier()) && toggleHlyTabHollyMagicSpellPower() && this.isEnabled.get()) {
			modifiers.put(AttributeRegistry.HOLY_SPELL_POWER.get() ,
					new AttributeModifier(HOLLY_SPELL_POWER_UUID , "" , getHlyTabHollyMagicSpellPower() / 100.0 ,
							AttributeModifier.Operation.MULTIPLY_TOTAL));
		}
		if (CuriosApi.getItemStackSlots(stack , slotContext.entity()).containsKey(slotContext.identifier()) && toggleHlyTabManaRegen() && this.isEnabled.get()) {
			modifiers.put(AttributeRegistry.MANA_REGEN.get() ,
					new AttributeModifier(MANA_REGEN_UUID , "" , getHlyTabManaRegen() / 100.0 ,
							AttributeModifier.Operation.MULTIPLY_TOTAL));
		}
		if (CuriosApi.getItemStackSlots(stack , slotContext.entity()).containsKey(slotContext.identifier()) && toggleHlyTabEvocationMagicResist() && this.isEnabled.get()) {
			modifiers.put(AttributeRegistry.EVOCATION_MAGIC_RESIST.get() ,
					new AttributeModifier(EVOCATION_MAGIC_RESIST_UUID , "" , -getHlyTabEvocationMagicResist() / 100.0 ,
							AttributeModifier.Operation.MULTIPLY_TOTAL));
		}
