...
public class EnderTabletBase extends ItemBaseClass {
	...
	public EnderTabletBase(Properties properties , String tooltip , Supplier<Boolean> enabled , GlintRenderTypes glintType) {
	...
	public @NotNull Component getName(@NotNull ItemStack stack) {
		...
		TextColor color = TextColor.fromRgb(0xA239E8); //
		...
		return new ICapabilityProvider() {
			...
			private final LazyOptional<ICurio> lazyCurio = LazyOptional.of(() -> new ICurio() {
				...
				public List<Component> getAttributesTooltip(List<Component> tt) {
					...
					List<Component> tooltips = ICurio.super.getAttributesTooltip(tt);
					return EnderTabletBase.this.getAttributesTooltip(replaceTooltips(tooltips));
				}
	...
	public void appendHoverText(@NotNull ItemStack stack , @Nullable Level worldIn , @NotNull List<Component> tooltip , @NotNull TooltipFlag flagIn) {
		...
		//================================================================ Colors
		TextColor color = TextColor.fromRgb(0xC062FF);
		TextColor color2 = TextColor.fromRgb(0xC891EC);
		TextColor loreColor = TextColor.fromRgb(0xEFD8FF);
		TextColor Value = TextColor.fromRgb(0xDDABFF);
		...
		MutableComponent eldritch = Component.translatable("school." + DivineArtifacts.MODID + ".ender").withStyle(s -> s.withColor(color2));

		//================ Lore by Amadhe, Magician of the Wiggle

		MutableComponent lore = Component.translatable("lore." + DivineArtifacts.MODID + ".ender").withStyle(s -> s.withColor(loreColor));
		MutableComponent lore2 = Component.translatable("lore2." + DivineArtifacts.MODID + ".ender").withStyle(s -> s.withColor(loreColor));
		MutableComponent lore3 = Component.translatable("lore3." + DivineArtifacts.MODID + ".ender").withStyle(s -> s.withColor(loreColor));
		MutableComponent lore4 = Component.translatable("lore4." + DivineArtifacts.MODID + ".ender").withStyle(s -> s.withColor(loreColor));
		MutableComponent lore5 = Component.translatable("lore5." + DivineArtifacts.MODID + ".ender").withStyle(s -> s.withColor(loreColor));
		MutableComponent lore6 = Component.translatable("lore6." + DivineArtifacts.MODID + ".ender").withStyle(s -> s.withColor(loreColor));
